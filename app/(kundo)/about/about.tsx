	...
	return (
		<Page className="flex flex-col gap-24 w-full mx-auto mt-0">
					...
					<div className="md:col-span-7 flex flex-col gap-10">
						...
						{whatWeDo.capabilities?.length ? (
							<div className="flex flex-col gap-8 border-t border-white/10 pt-8">
								{whatWeDo.capabilities.map((capability) => (
									<div key={capability._key} className="flex flex-col gap-2">
										{capability.title ? (
			...
			{prefooterAsset?.url || prefooter?.tagline ? (
				<section className="relative overflow-hidden rounded-[24px] border border-white/10 bg-[#050505] py-24 sm:py-28 md:py-32 flex flex-col items-center justify-center text-center gap-8">
					{prefooterAsset?.url ? (
						<Asset
							filetype={prefooterAsset.filetype}
							src={prefooterAsset.url}
							className="absolute inset-0 -z-10 rounded-none [&>div:last-child]:hidden"
							style={{
								filter: "grayscale(100%)",
								maskImage:
									"linear-gradient(to top, rgba(0,0,0,1) 20%, rgba(0,0,0,0.25) 70%, transparent 100%)",
							}}
						/>
					) : null}
					<div className="relative z-10 flex flex-col gap-4 items-center">
						{prefooter?.tagline ? (
							<span className="text-primary text-4xl sm:text-5xl md:text-6xl">
								{prefooter.tagline}
							</span>
						) : null}
					</div>
